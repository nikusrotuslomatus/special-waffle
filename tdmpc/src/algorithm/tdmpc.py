...
class TDMPC():
	...
	def __init__(self, cfg):
		...
		self._mi_sampler = None
		self._mi_init_failed = False
		self._last_plan_metrics = {
			'best_nf_return': float('nan'),
	...
	def _init_mi_sampler(self):
		"""Lazily initialize MI sampler if enabled and available."""
		if self._mi_sampler is not None or self._mi_init_failed:
			return
		try:
			from mi_sampler import ConditionalFlowSampler
		except Exception:
			self._mi_init_failed = True
			return
		try:
			self._mi_sampler = ConditionalFlowSampler(
			...
			if mi_path and os.path.isfile(mi_path):
				state = torch.load(mi_path, map_location=self.device)
				self._mi_sampler.load_state_dict(state)
			self._mi_sampler.eval()
		except Exception:
			self._mi_sampler = None
			self._mi_init_failed = True
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		...
		z = z0.repeat(self.cfg.num_samples+num_pi_trajs, 1)
		mean = torch.zeros(horizon, self.cfg.action_dim, device=self.device)
		std = 2*torch.ones(horizon, self.cfg.action_dim, device=self.device)
		if not t0 and hasattr(self, '_prev_mean'):
			mean[:-1] = self._prev_mean[1:]

		# Optional MI warm-start (sampler -> pick best -> refine)
		if getattr(self.cfg, 'use_mi_warmstart', False):
			self._init_mi_sampler()
			if self._mi_sampler is not None:
				with torch.no_grad():
					mi_num_samples = getattr(self.cfg, 'mi_num_samples', 128)
					u_nf, _ = self._mi_sampler.sample(z0, num_samples=mi_num_samples)
					u_nf = u_nf.squeeze(0)
					if u_nf.shape[1] > horizon:
						u_nf = u_nf[:, :horizon]
					elif u_nf.shape[1] < horizon:
						pad = torch.zeros((u_nf.shape[0], horizon - u_nf.shape[1], u_nf.shape[2]), device=u_nf.device)
						u_nf = torch.cat([u_nf, pad], dim=1)

					u_nf_t = u_nf.permute(1, 0, 2)
					z_nf = z0.repeat(u_nf.shape[0], 1)
					value_nf = self.estimate_value(z_nf, u_nf_t, horizon).nan_to_num_(0)
					best_idx = torch.argmax(value_nf.squeeze(1))
					best_nf_value = value_nf[best_idx].item()

					seed_value = self.estimate_value(z0, mean.unsqueeze(1), horizon).item()
		...
		for i in range(self.cfg.iterations):
			...
			if i == 0:
				best_gauss_value = value.max().item()
		...
		final_mean_return = self.estimate_value(z0, mean.unsqueeze(1), horizon).item()
		...
		self._last_plan_metrics = {
			'best_nf_return': float(best_nf_value),
